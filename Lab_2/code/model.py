...
class DbModel:
    ...
    def __init__(self):
        ...
        self.password = "123"
        try:
            self.__context = psycopg2.connect(host=self.host, database=self.database, user=self.user,
    ...
    def get_table_data(self, table_name):
        ...
        try:
            cursor.execute(
                sql.SQL('SELECT * FROM {}').format(sql.Identifier(table_name), sql.SQL(id_column)))
        except Exception as _ex:
            print("[INFO] Error while working with PostgreSQL", _ex)
    ...
    def insert_data(self, table_name, values):
        ...
        columns = columns[:-3] + '")'
        self.__cursor.execute(
            sql.SQL('INSERT INTO {} {} VALUES (' + line[:-1] + ')').format(sql.Identifier(table_name),
                                                                           sql.SQL(columns)),
            values)
    ...
    def change_data(self, table_name, values):
        ...
        for key in values:
            if values[key]:
                line += key + '=%(' + key + ')s,'
        self.__cursor.execute(
            sql.SQL('UPDATE {} SET ' + line[:-1] + ' WHERE {} ').format(sql.Identifier(table_name), sql.SQL(condition)),
            values)
        self.__context.commit()

    def delete_data(self, table_name, value, cond):
        self.__cursor.execute(
            sql.SQL('DELETE FROM {} WHERE {} = {}').format(sql.Identifier(table_name), sql.Identifier(value),
                                                           sql.SQL(cond)))
        self.__context.commit()
